document.addEventListener('DOMContentLoaded', () => {
    ...
    const cakeSvgWrapper = document.getElementById('cake-svg-wrapper');
    ...
    // Injected dynamically so SMIL timings begin from injection moment
    // ====================================================
    ...
    C177.668,5.569,177.667,21.571,173.667,21.571z">
            <animate id="bizcocho_1" attributeName="d" calcMode="spline"
                ...
                begin="0.5s" dur="0.8s" fill="freeze" values="
    ...
    c46.385,0,97.539,0,147.334,0C177.668,411.567,177.667,427.569,173.667,427.569z
        ...
        <!-- CREAM / ICING (drips down the sides after all layers land) -->
    ...
    c-7.606-0.447-6.058-37.895-20.62-23.333c-10.167,10.166-15.972-0.747-25,12C119.547,443.568,121.798,416.515,111.547,415.233z
        ...
        <!-- Cake stand/plate line -->
        <rect x="10" y="475.571" fill="#fefae9" width="180" height="4" rx="2" />
    </svg>`;
    ...
    function openCard() {
        ...
        activeTimeouts.push(setTimeout(() => {
            ...
            activeTimeouts.push(setTimeout(() => {
                birthdayCard.classList.add('fade-in');

                // Inject the SVG cake NOW so SMIL animations begin from this moment
                // Append SVG after the velas element (don't use innerHTML which would destroy velas)
                cakeSvgWrapper.insertAdjacentHTML('beforeend', cakeSvgTemplate);

                // SMIL animation timeline (from injection):
                // 0.5s: bizcocho_1 starts (0.8s dur) → ends at 1.3s
                // 1.3s: relleno_1 starts (0.5s dur) → ends at 1.8s
                // 1.8s: bizcocho_2 starts (0.5s dur) → ends at 2.3s
                // 2.3s: relleno_2 starts (0.5s dur) → ends at 2.8s
                // 2.8s: bizcocho_3 starts (0.3s dur) → ends at 3.1s
                // 3.1s: crema starts (2.0s dur) → ends at 5.1s
                ...
                activeTimeouts.push(setTimeout(() => {
                    ...
                    if (catImg) {
                        ...
                        catImg.style.transform = 'translateX(-75%) translateY(0)';
                    }
                    ...
                    confetti({
                        ...
                        spread: 60,
                        origin: { x: 0.5, y: 0.55 },
                        colors: ivyConfettiColors,
                        ticks: 200,
                        gravity: 1.2,
                        scalar: 0.9
                    });
                }, 5500)); // after cream animation finishes (~5.1s + small buffer)
    ...
    envelope.addEventListener('click', (e) => {
        if (e.target !== openCardBtn && !openCardBtn.contains(e.target)) {
            openCard();
        }
    });
    ...
    // 4. CONFETTI BLASTS
    // ====================================================
    function triggerFinalConfettiBlast() {
        const duration = 4.5 * 1000;
        const animationEnd = Date.now() + duration;
        const defaults = { startVelocity: 25, spread: 360, ticks: 120, zIndex: 100, colors: ivyConfettiColors };
        ...
        setTimeout(() => {
            ...
            (function frame() {
                confetti({
                    particleCount: 7, angle: 60, spread: 65,
                    origin: { x: 0, y: 0.8 }, colors: ivyConfettiColors
                });
                confetti({
                    particleCount: 7, angle: 120, spread: 65,
                    origin: { x: 1, y: 0.8 }, colors: ivyConfettiColors
                });
    ...
    velas.addEventListener('click', () => {
        ...
        if (instructionBlow) {
            instructionBlow.classList.remove('active');
            activeTimeouts.push(setTimeout(() => {
                instructionBlow.style.display = 'none';
            }, 400));
        }
        ...
        // Explode massive birthday confetti!
        triggerFinalConfettiBlast();
    });

    // ====================================================
    // 6. RESET SYSTEM (Replay)
    // ====================================================
    resetBtn.addEventListener('click', () => {
        ...
        // Visual resets
        velas.classList.remove('lit', 'blown-out');
        wishMessage.classList.remove('revealed');
        if (instructionBlow) {
            instructionBlow.style.display = 'flex';
            activeTimeouts.push(setTimeout(() => {
                instructionBlow.classList.add('active');
            }, 50));
        }
        ...
        if (catImg) {
            ...
            catImg.style.transform = 'translateX(-75%) translateY(30px)';
        }
        cakeSvgWrapper.classList.remove('fade-out-cake');

        // Remove injected SVG cake (will be re-injected on next open)
        // Remove only the injected SVG, keep the velas element
        const oldSvg = cakeSvgWrapper.querySelector('svg');
        if (oldSvg) oldSvg.remove();

        birthdayCard.classList.remove('fade-in');
